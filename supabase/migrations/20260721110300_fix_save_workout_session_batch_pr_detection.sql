-- =============================================================================
-- Phase 2 — Module C: fix duplicate-achievement logging for multi-set
-- ascending/pyramid payloads in save_workout_session_v1
-- Fixes: public.save_workout_session_v1, introduced in
-- supabase/migrations/20260721110000_create_workout_save_and_pr_rpcs.sql
--
-- Per this project's migration convention, 20260721110000 is already applied
...
-- =============================================================================
-- The original save_workout_session_v1 ran PR detection INLINE, once per set,
-- inside the per-set upsert loop -- i.e. it evaluated
-- private._strength_pr_apply_or_recompute sequentially as each set in the
-- payload was written, in payload order. For a single set per call this is
-- fine (it is exactly Module A's one-candidate-per-activity model). But a
-- realistic strength payload commonly contains MULTIPLE working sets of the
-- SAME exercise in one call -- e.g. an ascending/pyramid squat session
-- (100kg, then 105kg, then 110kg, three working sets, one save_workout_
-- session_v1 call at end-of-workout sync). Processed sequentially, each set
-- genuinely "beats the cache" at ITS turn in the loop (100 beats null, 105
-- beats 100, 110 beats 105) and each logs its own strength_achievements row
-- -- three achievement rows for what a user experiences as one PR-setting
-- lift. This is NOT a rare concurrency edge case (contrast Module A's
-- documented §2.6 concurrent-achievement risk, which requires genuine
-- multi-device simultaneity) -- it is deterministic, single-request,
-- single-device behavior that fires on every ordinary ramping-set session.
-- Live-reproduced: scripts/verify-save-workout-session.mjs's Case 1 (a
-- 100kg then 105kg set in one call) asserted "first-ever PR" should log
-- exactly one achievement per metric and instead got two (100kg logged,
-- then immediately superseded by 105kg in the same call).
--
-- Root cause: PR detection's actual unit of comparison should be "the best
-- qualifying value THIS SAVE CALL contributes per (exercise_ref, metric)",
-- not "each individual set". §4.3 itself describes the cost bound as
-- O(#exercises-in-session × #metrics) -- explicitly per-exercise, not
-- per-set -- which the original per-set-loop implementation did not actually
-- honor (it was O(#sets × #metrics)).
--
-- =============================================================================
-- THE FIX
-- =============================================================================
-- Move PR detection OUT of the per-set upsert loop entirely. After all sets
-- in the payload are upserted (so estimated_1rm_kg etc. are already
-- server-computed and committed within this transaction), run one query per
-- metric across ONLY the sets this call just wrote (tracked via v_set_ids),
-- grouped by (exercise_id, custom_exercise_id) with `DISTINCT ON ... ORDER
-- BY <metric column> DESC` to pick the single best qualifying set per
-- exercise per metric -- then call private._strength_pr_apply_or_recompute
-- exactly once per (exercise_ref, metric) pair, attributing the achievement
-- to whichever specific set actually holds that call's best value. This
-- restores the O(#exercises-in-call × #metrics) bound §4.3 describes and
-- produces exactly one achievement per exercise per metric per save call --
-- matching how a user actually experiences "I hit a new PR this session",
-- not "I hit N transient micro-PRs on the way to my real one".
--
-- Sets NOT in this call's payload (already-committed, untouched) are
-- correctly excluded -- nothing about them changed, so re-evaluating them
-- would be wasted work; if a payload-included set's edit or tombstone
-- demotes an existing record holder that ISN'T in this call's candidate set,
-- the AFTER UPDATE trigger (trg_workout_set_logs_pr_recompute_on_change,
-- unaffected by this migration) still reconciles that correctly.
--
-- `CREATE OR REPLACE FUNCTION` with an unchanged signature -- no
-- DROP/CASCADE needed, no caller needs to change.
--
-- Live re-verified after this fix: scripts/verify-save-workout-session.mjs
-- passes all cases in full, including a fresh 2-set ascending payload now
-- logging exactly the 3 achievements for the winning (105kg) set, not 6
-- across both sets.
--
-- ROLLBACK: see supabase/migrations/rollbacks/20260721110300_fix_save_workout_session_batch_pr_detection.sql
-- (restores the original per-set inline detection from 20260721110000, i.e.
-- reintroduces the bug -- exists for convention-consistency only).
-- =============================================================================
...
begin
  ...
  begin
    ...
    -- cheap at this bounded per-session scale). PR detection is now a
    -- SEPARATE batched pass below, not inline here (fixed by this
    -- migration -- see header).
    v_set_ids := array(select (elem ->> 'id')::uuid from jsonb_array_elements(p_sets) elem);
    ...
    -- ---------------------------------------------------------------------
    -- Batched PR detection (§4.3, fixed by this migration): ONE candidate
    -- per (exercise_ref, metric) -- the best qualifying set THIS CALL wrote
    -- (v_set_ids), not one evaluation per set. DISTINCT ON picks the top set
    -- per exercise_ref per metric; each is compared against the cache via a
    -- single indexed point lookup, exactly O(#exercises-in-call × #metrics)
    -- per §4.3.
    -- ---------------------------------------------------------------------
    for v_pr in
      select distinct on (wsl.exercise_id, wsl.custom_exercise_id)
        wsl.exercise_id, wsl.custom_exercise_id, wsl.weight_kg as value,
        wsl.unit_weight_snapshot, wsl.id as source_set_log_id
      from public.workout_set_logs wsl
      where wsl.id = any (v_set_ids)
        ...
        and (
          (wsl.exercise_id is not null and exists (select 1 from public.exercises e where e.id = wsl.exercise_id and e.is_weighted))
          or (wsl.custom_exercise_id is not null and exists (select 1 from public.custom_exercises c where c.id = wsl.custom_exercise_id and c.is_weighted))
        )
      order by wsl.exercise_id, wsl.custom_exercise_id, wsl.weight_kg desc
    loop
      perform private._strength_pr_apply_or_recompute(
        v_user_id, v_pr.exercise_id, v_pr.custom_exercise_id, 'heaviest_weight',
        v_pr.value, v_pr.unit_weight_snapshot, v_pr.source_set_log_id, p_id, p_occurred_at
      );
    end loop;

    for v_pr in
      select distinct on (wsl.exercise_id, wsl.custom_exercise_id)
        wsl.exercise_id, wsl.custom_exercise_id, wsl.estimated_1rm_kg as value,
        wsl.unit_weight_snapshot, wsl.id as source_set_log_id
      from public.workout_set_logs wsl
      where wsl.id = any (v_set_ids)
        and wsl.deleted_at is null
      ...
      order by wsl.exercise_id, wsl.custom_exercise_id, wsl.estimated_1rm_kg desc
    loop
      perform private._strength_pr_apply_or_recompute(
        v_user_id, v_pr.exercise_id, v_pr.custom_exercise_id, 'estimated_1rm',
        v_pr.value, v_pr.unit_weight_snapshot, v_pr.source_set_log_id, p_id, p_occurred_at
      );
    end loop;
    ...
    for v_pr in
      select distinct on (wsl.exercise_id, wsl.custom_exercise_id)
        wsl.exercise_id, wsl.custom_exercise_id, (wsl.reps * wsl.weight_kg) as value,
        wsl.unit_weight_snapshot, wsl.id as source_set_log_id
      from public.workout_set_logs wsl
      where wsl.id = any (v_set_ids)
        and wsl.deleted_at is null
      ...
      order by wsl.exercise_id, wsl.custom_exercise_id, (wsl.reps * wsl.weight_kg) desc
    loop
      perform private._strength_pr_apply_or_recompute(
        v_user_id, v_pr.exercise_id, v_pr.custom_exercise_id, 'best_set_volume',
        v_pr.value, v_pr.unit_weight_snapshot, v_pr.source_set_log_id, p_id, p_occurred_at
      );
    end loop;
    ...
    for v_pr in
      select distinct on (wsl.exercise_id, wsl.custom_exercise_id)
        wsl.exercise_id, wsl.custom_exercise_id, wsl.reps::numeric as value,
        null::text as unit_weight_snapshot, wsl.id as source_set_log_id
      from public.workout_set_logs wsl
      where wsl.id = any (v_set_ids)
        and wsl.deleted_at is null
        and wsl.set_type = 'working'
        ...
        and wsl.reps is not null
        and (
          (wsl.exercise_id is not null and exists (select 1 from public.exercises e where e.id = wsl.exercise_id and e.is_bodyweight))
          or (wsl.custom_exercise_id is not null and exists (select 1 from public.custom_exercises c where c.id = wsl.custom_exercise_id and c.is_bodyweight))
        )
      order by wsl.exercise_id, wsl.custom_exercise_id, wsl.reps desc
    loop
      perform private._strength_pr_apply_or_recompute(
        v_user_id, v_pr.exercise_id, v_pr.custom_exercise_id, 'max_reps',
        v_pr.value, v_pr.unit_weight_snapshot, v_pr.source_set_log_id, p_id, p_occurred_at
      );
    end loop;
...
comment on function public.save_workout_session_v1(
  ...
  'set''s own id inside p_sets. PR detection is batched per (exercise_ref, '
  'metric) across this call''s sets, not evaluated per-set (fixed in '
  '20260721110300). Returns {"data": {...}} on success or '
